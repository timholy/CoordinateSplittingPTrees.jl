...
where `Δx = x - b`.
"""
function fit_quadratic(f::Function, box::Box{2})
    c, g, Q, b, y, prec, firstmatch = fit_quadratic_native(f, box)
    return canonicalize(c, g, Q, b, y, prec)
end
fit_quadratic(box::Box) = fit_quadratic(value, box)
...
"""
function fit_quadratic_native(f::Function, box::Box{2})
    c, g, b, y, prec, firstmatch = fit_poly1(f, box)
    Q = coefficients_p(f, box)
    fit_poly2_diag!(f, Q, box, c, g, b, y, prec)
    return c, g, Q, b, y, prec, firstmatch
end
fit_quadratic_native(box::Box{2}) = fit_quadratic_native(value, box)
...
function fit_poly1(f::Function, box::Box{2,T}) where T
    box = getleaf(box)
    n = ndims(box)
...
function fit_poly2_diag!(f::Function, Q, box::Box{2}, c, g, b, y, prec)
